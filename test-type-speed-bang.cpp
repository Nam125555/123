...
void Nhap_chuong_nuoi()
{
    ...
    for(int i=0;i<so_luong_chuong;i++)
    {
        ...
        ofs<<danhsach_machuong_temp[i].ma_chuong<<" "<<danhsach_machuong_temp[i].so_luong<<" "<<danhsach_machuong_temp[i].ngay_nhan<<" "<<danhsach_machuong_temp[i].giong<<endl;

    }
    ofs.close();
}

void in_file_chuong_nuoi()
{
    ifstream ifs("ten_file.txt");
    if (ifs.is_open())
    {
        for (int i = 0; i < so_luong_chuong; i++)
        {
            if (!getline(ifs,danhsach_machuong_temp[i].ma_chuong,' ')) {
                cout << "Loi: khong the doc ma chuong tai i = " << i << endl;
            }
            if (!getline(ifs,danhsach_machuong_temp[i].so_luong,' ')) {
                cout << "Loi: khong the doc so luong tai i = " << i << endl;
            }
            if (!getline(ifs,danhsach_machuong_temp[i].ngay_nhan,' ')) {
                cout << "Loi: khong the doc ngay nhan tai i = " << i << endl;
            }
            if (!getline(ifs,danhsach_machuong_temp[i].giong)) {
                cout << "Loi: khong the doc giong tai i = " << i << endl;
            }

            cout << "ma_chuong = " << danhsach_machuong_temp[i].ma_chuong << ", ";
            cout << "so_luong = " << danhsach_machuong_temp[i].so_luong << ", ";
            cout << "ngay_nhan = " << danhsach_machuong_temp[i].ngay_nhan << ", ";
            cout << "giong = " << danhsach_machuong_temp[i].giong << endl;
        }
        ifs.close();
    }
}
...
int main()
{
    char fileName[] = "tenfile.txt";

    SetWindowSize(130, 100);
    SetScreenBufferSize(130, 100);
